...
function parseArgs(argv) {
  const args = { feeds: [], item: 0, category: '', players: [], publishDate: '', out: DEFAULT_OUT_DIR, selfTest: false };
...
async function fetchFeed(url) {
  let res;
  try {
    res = await fetch(url, { headers: { Accept: 'application/rss+xml, application/atom+xml, application/xml, text/xml;q=0.9, */*;q=0.1' } });
  } catch (err) {
...
function buildArticle(item, options) {
  ...
  const entities = dedupeEntities(options.players, title + ' ' + summary);
  ...
  return { title, slug: slugify(publishDate + '-' + title), publishDate, category: inferCategory(item, options.category), excerpt, author: 'FSN Desk', entities, body };
}
...
async function generate(options) {
  ...
  if (!item) throw lastError || new Error('[generate-editorial] no public feed yielded an article.');
...
async function runSelfTest() {
  ...
  try {
    const file = await generate({ feeds:['http://127.0.0.1:' + server.address().port + '/feed.xml'], item:0, category:'', players:[parsePlayer('CeeDee Lamb|WR')], publishDate:'', out:tmp });
    ...
    try { buildArticle(parseFeed(fixtureXml())[0], { players:[parsePlayer('Ghost Player|QB')], category:'', publishDate:'' }); }
...
async function main() {
  ...
  try {
    ...
    console.log('[generate-editorial] done. Run "npm run build:blog" to compile ' + path.relative(ROOT, file) + '.');
  } catch (err) {
