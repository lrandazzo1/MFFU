...
const handler = require('../api/blog/articles-publish');
...
async function call(body, options = {}) {
  ...
  try {
    ...
    const res = fakeRes();
    await handler({ method: options.method || 'POST', headers, body }, res);
    return { res, db };
  } finally {
...
test('a three-tier publish writes every tier and mirrors the legacy columns', async () => {
  ...
  assert.equal(row.author, 'FFU News Desk');
...
test('a legacy caller sending title and content_markdown gets the same row', async () => {
  ...
  assert.equal(row.author, 'FFU News Desk', 'the byline defaults to the desk');
});
