...
function scope(input) {
  ...
  if (!['espn', 'sleeper', 'yahoo'].includes(provider) ||
      !(provider === 'yahoo' ? /^\d{1,8}\.l\.\d{1,20}$/.test(league) : /^\d{1,20}$/.test(league)) ||
...
async function espnRead(req, url) {
  ...
  if (status < 200 || status >= 300 || !body || typeof body !== 'object') {
    throw Object.assign(new Error('ESPN transaction read failed (HTTP '+status+')'), { status:[400,401,403,404,429].includes(status) ? status : 502 });
  }
  return body;
}
...
function espnSnapshot(data, s) {
  ...
  if (data.transactions == null) {
    console.warn('[TransactionWire] ESPN transaction detail omitted for '+s.key,new Error('ESPN_TRANSACTIONS_UNAVAILABLE'));
    warnings.push('ESPN_TRANSACTIONS_UNAVAILABLE');
  } else if (!Array.isArray(data.transactions)) throw new Error('Invalid ESPN transaction collection');
  const players = {}, teams = {};
  ...
  const events = list(data.transactions).filter(t => t.status === 'EXECUTED' && ['WAIVER','FREEAGENT','TRADE','DROP'].includes(t.type)).map(t => ({
    id:String(t.id || ''), kind:t.type === 'TRADE' ? 'trade' : t.type === 'WAIVER' ? 'waiver' : 'move',
    at:number(t.processDate), week:number(t.scoringPeriodId),
  ...
  return { players, teams, events, warnings, week:Number(data.status && data.status.currentMatchupPeriod || data.scoringPeriodId) };
}
...
function sleeperSnapshot(league, rosters, users, playersRaw, transactions, s) {
  ...
  if (!playersRaw || typeof playersRaw !== 'object' || Array.isArray(playersRaw)) throw new Error('Sleeper player index unavailable');
  const players = {}, teams = {};
  ...
  const events = transactions.filter(t => t.status === 'complete' && ['trade','waiver','free_agent'].includes(t.type)).map(t => {
    ...
    return { id:String(t.transaction_id || ''), kind:t.type === 'trade' ? 'trade' : t.type === 'waiver' ? 'waiver' : 'move',
      at:number(t.status_updated), week:number(t.leg),
...
function yahooSnapshot(metadata, rosterData, transactionData, s) {
  ...
  const events = transactionData.flatMap(d => nodes(d,'transaction')).map(raw => {
    ...
    return { id:String(t.transaction_key || ''), kind:t.type === 'trade' ? 'trade' : 'move',
...
async function ingest(s, req, since) {
  ...
  if (s.provider === 'sleeper') {
    ...
    const transactions = [];
    const weeks = Array.from({length:current-Math.max(1,current-2)+1},(_,i)=>Math.max(1,current-2)+i);
    const batches = await Promise.all(weeks.map(week=>json(base+'/transactions/'+week)));
