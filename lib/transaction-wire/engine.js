...
const OPEN = {
  waiver:[
    'The waiver order moved. Somebody wanted an answer badly enough to make the roster decision official.',
    'The claim cleared. The excuses are still pending. This is the part of the season when roster conviction starts leaving evidence.',
    ...
    'One processed claim, one fresh obligation. Winning waiver priority is the easy half of a roster decision.',
    'The front office has made its move. The rest of the league gets to keep the transaction on file until the scoreboard tells it what to do with it.',
  ...
  trade:[
    ...
    'The league’s personnel map has been redrawn. A completed trade creates an entry on both sides, with nowhere to hide the players who changed hands.',
...
const CLOSE = [
  ...
  'The front office has changed the available answers. The weekly score will decide whether it asked the right question. Until then, the transaction belongs to everybody in the league.',
];
...
function generate(s, event, snapshot, observedAt) {
  ...
  if (event.kind === 'injury') {
    ...
    headline = event.kind === 'trade'
      ...
      : `${esc(teams[0].name.toUpperCase())} ${focus.to ? 'ADDS' : 'CUTS'} ${esc(name(focus.playerId).toUpperCase())}`;
    ...
    event.assets.forEach(a => {
      if (a.type !== 'pick' || !(a.season > 0 && a.round > 0)) throw new Error('Invalid trade asset');
