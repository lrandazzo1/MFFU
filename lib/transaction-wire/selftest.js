...
/* `at` stamps the fixture's transactions. It defaults to the pinned `now` above,
   which keeps every build()/sync() assertion in this file deterministic. Tests
   that drive the HTTP route cannot inject a clock — the route calls sync()
   without dependencies, so it reads the real Date.now() — and those pass the
   live clock in here instead. Without that, the pinned fixture ages out of the
   engine's 14-day ingestion window and the route returns zero articles, which
   is a stale fixture rather than a defect in the route. */
function fixture(at) {
  const stamp = at == null ? now : at;
  return sleeperSnapshot({league_id:'123',season:'2026',settings:{leg:2}},[
    ...
    {transaction_id:'ok',status:'complete',type:'waiver',status_updated:stamp-1000,leg:2,adds:{2:1},settings:{waiver_bid:0},waiver_budget:[{amount:15,sender:2,receiver:1}]},
    {transaction_id:'failed',status:'failed',type:'waiver',status_updated:stamp-1000,leg:2,adds:{2:2},settings:{waiver_bid:99}},
    {transaction_id:'pending',status:'pending',type:'trade',status_updated:stamp-1000,leg:2,adds:{3:1},drops:{3:2}},
  ],s);
}
...
test('only completed moves publish; claim economics are ignored while scoring and escaping are preserved',()=>{
  const snapshot = fixture(), result = build(s,snapshot,null,now,now-86400000);
  assert.equal(result.articles.length,1);
  assert.equal(snapshot.events[0].bid,undefined);
  assert.deepEqual(snapshot.events[0].assets,[]);
  const a = result.articles[0];
  assert.ok(!a.headline.includes('$'));
  assert.ok(!/FAAB|waiver bid|winning bid|budget/i.test(a.paragraphs.join(' ')));
  assert.ok(!/FAAB|waiver bid|winning bid|budget/i.test(JSON.stringify(a.numbers)));
  ...
  assert.ok(a.paragraphs.length >= 5);
...
test('full multi-team trades retain every player and draft pick while rejecting budget assets',()=>{
  const snap = fixture();
  snap.events = [{id:'trade',kind:'trade',at:now-1,week:2,moves:[{playerId:'1',from:'1',to:'2'},{playerId:'3',from:'2',to:'1'}],assets:[{type:'pick',season:2027,round:2,original:'1',from:'1',to:'2'}]}];
  const a = build(s,snap,null,now,0).articles[0];
  assert.equal(a.numbers.rows.length,3);
  assert.match(a.paragraphs.join(' '),/2027 round 2/);
  assert.match(a.paragraphs.join(' '),/Receiver Three/);
  const unsupported = structuredClone(snap);
  unsupported.events[0].assets.push({type:'budget',amount:15,from:'2',to:'1'});
  assert.equal(build(s,unsupported,null,now,0).articles.length,0);
});
...
test('cron rejects missing credentials and wrong HTTP methods',async()=>{
  ...
  let res = response(); await handler({method:'GET',headers:{},query:{action:'dispatch'}},res); assert.equal(res.code,401);
  res = response(); await handler({method:'POST',headers:{},query:{action:'dispatch'}},res); assert.equal(res.code,405);
  delete require.cache[route];
});
...
test('HTTP GET and POST succeed in live-only mode when Supabase configuration is missing',async()=>{
  ...
  // The route resolves its own clock, so the fixture is stamped to it.
  providers.ingest=async()=>fixture(Date.now());
