...
test('every archetype trigger selects its own archetype', () => {
  ...
  assert.equal(new Set(ARCHETYPE_CASES.map(([name]) => name)).size, 8, 'all eight are covered');
});
...
test('the template variant is deterministic and both phrasings are reachable', () => {
  ...
  for (const [name, over] of ARCHETYPE_CASES) {
    const a = sentenceFor(trackedRow({ ...over, player_id: '2' }), 2);   // even
    const b = sentenceFor(trackedRow({ ...over, player_id: '3' }), 2);   // odd
...
test('an archetype that names the opponent emboldens it too', () => {
  ...
  for (const [name, over] of ARCHETYPE_CASES) {
    ...
    for (const playerId of ['2', '3']) {
      const line = sentenceFor(trackedRow({ ...over, player_id: playerId, opponent_team: 'Gulf Current' }), 2);
...
test('no archetype outside GAME_WINNER claims the player won anything', () => {
  ...
  for (const [name, over] of ARCHETYPE_CASES) {
    ...
    for (const playerId of ['2', '3']) {
      const line = sentenceFor(trackedRow({ ...over, player_id: playerId }), 2);
      assert.equal(
