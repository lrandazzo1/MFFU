/* ============================================================================
   ...
   read in api/blog/articles.js.

   ---- WHY IT LIVES IN lib/ AND NOT api/ ----

   Vercel turns every file under `api/` into its own Serverless Function, and
   the Hobby plan allows twelve per deployment. This project already has
   exactly twelve. A thirteenth file here does not fail the build: the build
   succeeds and the DEPLOY fails at patchBuild with
   `exceeded_serverless_functions_per_deployment`, taking the whole site's
   production deployment down with it, which is how this module got moved.

   So the handler lives in lib/ and `api/blog/articles.js` dispatches to it on
   `?action=publish`, with a rewrite in vercel.json mapping the public
   `/api/blog/articles/publish` onto that. It is the same shape the project
   already uses for `/api/notifications-register`,
   `/api/transaction-wire-dispatch` and `/api/auth/yahoo/callback`, and it is
   why those exist rather than being separate files. `blog_articles` is RLS-protected with no
   ...
   directly) and this route.
     ...
     POST /api/blog/articles/publish        (rewritten to ?action=publish)
...
const { authorizedByCronSecret, cronSecretConfigured } = require('./dist/article-cron');
