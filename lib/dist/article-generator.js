    ...
    switch (row.outcome_flag) {
        case 'GAME_WINNER':
            return `${b(row.player_name)} won the matchup for ${b(row.owner_team)}. They trailed by ` +
                `${b(pts(Math.abs(row.entering_margin)))} before his game and finished ` +
                `${b(gap)} clear, with his ${b(pts(row.player_points) + ' pts')} covering the deficit.`;
        case 'GARBAGE_TIME_BLOWOUT':
            return `${b(row.player_name)} put up ${b(pts(row.player_points) + ' pts')} for ${b(row.owner_team)} in a ` +
                `game that was already gone: the lead was ${b(pts(row.entering_margin))} before he played ` +
                `and ${b(gap)} after. Unneeded stat-padding, nothing more.`;
        case 'VALIANT_LOSS':
            return `${b(row.player_name)} went for ${b(pts(row.player_points) + ' pts')} and ${b(row.owner_team)} ` +
                `lost anyway, by ${b(gap)}. A monster game, wasted.`;
        case 'DUD_COST_WIN':
            return `${b(row.owner_team)} led by ${b(pts(row.entering_margin))} before ` +
                `${b(row.player_name)} played, then lost by ${b(gap)}. He finished on ` +
                `${b(pts(row.player_points) + ' pts')} against ${article(row.projected_points)} ` +
                `${pts(row.projected_points)} point projection, and that gap is the matchup.`;
        default:
 ...
 */
const BOARD_PRIORITY = ['GAME_WINNER', 'DUD_COST_WIN', 'VALIANT_LOSS'];
