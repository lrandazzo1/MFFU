...
exports.defaultComposer = exports.DEFAULT_AUTHOR = exports.CATEGORY_BY_TYPE = exports.GAME_WINDOW_MS = exports.OUTCOME_FRAMING_RULE = exports.ARTICLE_TYPE_BY_DAY = void 0;
...
exports.archetypeFor = archetypeFor;
exports.templateVariant = templateVariant;
exports.rotateVariants = rotateVariants;
exports.sentenceFor = sentenceFor;
exports.decisiveRows = decisiveRows;
exports.boardRows = boardRows;
exports.liveStateOf = liveStateOf;
exports.hasPlayed = hasPlayed;
exports.previewMatchups = previewMatchups;
exports.orderPreviewMatchups = orderPreviewMatchups;
exports.leadSwing = leadSwing;
    ...
    if (request.article_type === 'friday_tnf_preview') {
        ...
        if (matchups.length) {
            ...
            for (const m of matchups) {
                lines.push(`- ${m.a.team} vs ${m.b.team}: ` +
            ...
            lines.push('', 'Lead with what has already happened. Build the article out of these matchups, not out of the', 'player list above, and never write a line of the form "player starts for team on a projection".');
        }
    }
...
function archetypeFor(row) {
    ...
    if (flag === 'GAME_WINNER' && primetime && deficit > 0 && margin > 0)
        return 'PRIMETIME_COMEBACK';
    // 2. A comeback that came down to a field goal.
    if (flag === 'GAME_WINNER' && margin <= 3)
        return 'RAZOR_THIN_COMEBACK';
    // 3. A comeback that overshot the deficit by half again or more.
    if (flag === 'GAME_WINNER' && deficit > 0 && points >= deficit * 1.5)
        return 'SINGLE_HANDED_OVERHAUL';
    // 4. Padding that turned a win into a demolition.
    if (flag === 'GARBAGE_TIME_BLOWOUT' && margin >= 25)
        return 'HEAVYWEIGHT_BLOWOUT';
    // 5. A huge number in a loss.
    if (flag === 'VALIANT_LOSS' && points >= 35)
        return 'WASTED_ERUPTION';
    // 6. A loss by a field goal or less.
    if (flag === 'VALIANT_LOSS' && margin <= 3)
        return 'HEARTBREAK_LOSS';
    // 7. Padding that kept a win comfortable rather than embarrassing.
    if (flag === 'GARBAGE_TIME_BLOWOUT' && margin >= 5 && margin < 25)
        return 'NECESSARY_INSURANCE';
    ...
    if (flag === 'GAME_WINNER' && deficit > 0)
        return 'STRAIGHT_COMEBACK';
 ...
 * row variant A.
 */
function templateVariant(row, week, occurrence = 0) {
    ...
    if (digits) {
        ...
        for (let i = 0; i < raw.length; i++)
            key = (key * 31 + raw.charCodeAt(i)) % 1000000007;
    }
 ...
 */
function rotateVariants(board, week) {
    const seeds = {};
    const seen = {};
    return board.map((row) => {
        const archetype = archetypeFor(row);
        if (seeds[archetype] === undefined)
            seeds[archetype] = templateVariant(row, week);
 ...
 * the same thing.
 */
function sentenceFor(row, week = 0, variant = templateVariant(row, week)) {
    const archetype = archetypeFor(row);
    const phrasing = {
...
exports.GAME_WINDOW_MS = 4 * 60 * 60 * 1000;
function liveStateOf(row, now) {
    ...
    if (Number.isFinite(clock) && Number.isFinite(kickoff) && kickoff > 0) {
        if (clock < kickoff)
            return 'PENDING';
        return clock < kickoff + exports.GAME_WINDOW_MS ? 'LIVE' : 'FINAL';
    }
    ...
    if ((Number(row && row.player_points) || 0) > 0)
        return 'LIVE';
    return 'UNKNOWN';
}
/** Whether this starter's points are on the board yet. */
function hasPlayed(row, now) {
...
function buildPreviewSide(team, starters, now) {
    const played = [];
    const pending = [];
    let projected = null;
    let scored = 0;
 ...
 * for it.
 */
function previewMatchups(rows, now) {
    const order = [];
    ...
    for (const id of order) {
        const sides = Array.from(byMatchup.get(id).entries());
        if (sides.length !== 2) {
            console.warn('[ArticleGenerator] matchup ' + id + ' resolved ' + sides.length +
                ' sides, so no preview storyline is built for it', new Error('PREVIEW_SIDE_COUNT'));
            continue;
        }
        ...
        const finished = (side) => side.starters.length > 0 && side.starters.every((row) => liveStateOf(row, now) === 'FINAL');
        ...
        out.push({
            ...
            projected_margin: a.projected == null || b2.projected == null ? null : round2(Math.abs(a.projected - b2.projected)),
        });
    }
    return out;
}
 ...
 */
function orderPreviewMatchups(matchups) {
    const gap = (m) => {
    ...
    const onTheBoard = (m) => m.a.scored > 0 || m.b.scored > 0;
    return matchups.slice().sort((x, y) => {
        if (x.live !== y.live)
            return x.live ? -1 : 1;
        if (x.live && onTheBoard(x) !== onTheBoard(y))
            return onTheBoard(x) ? -1 : 1;
 ...
 */
function previewVariants(matchups, week) {
...
const starterWord = (count) => (count === 1 ? 'starter' : 'starters');
/** Where a matchup with points already on the board actually stands. */
function liveStanding(m, variant) {
 ...
 */
function scorerLine(row, state, variant) {
...
function swingLine(swing, variant) {
...
function projectedStanding(m, variant) {
    ...
    const favourite = m.a.projected >= m.b.projected ? m.a : m.b;
 ...
 * gap rests on.
 */
function previewBlock(m, variant, now) {
    const sentences = [];
    if (m.live) {
        sentences.push(liveStanding(m, variant));
        const top = topScorer(m);
        if (top)
            sentences.push(scorerLine(top, liveStateOf(top, now), variant));
        const swing = leadSwing(leaderFirst(m)[0]);
        if (swing)
            sentences.push(swingLine(swing, variant));
        if (!m.complete) {
            const toCome = toComeLine(m, variant);
            if (toCome)
                sentences.push(toCome);
        }
    }
    else {
...
exports.DEFAULT_AUTHOR = 'FSN News Desk';
/**
 ...
 */
function impactSummary(rows, articleType, options = {}) {
    ...
    if (articleType === 'friday_tnf_preview') {
        ...
        if (!matchups.length)
            return 'Lineups are locked and no matchup pairing resolved yet.';
 ...
 */
function composePreview(request, heading, slate) {
    ...
    const matchupWord = (count) => (count === 1 ? 'Matchup' : 'Matchups');
    ...
    if (!matchups.length) {
        ...
        console.warn('[ArticleGenerator] no head to head matchups resolved for the week ' + request.week +
            ' preview, so it publishes without a matchup board', new Error('NO_PREVIEW_MATCHUPS'));
        body.push(`${slate.length} ${slate.length === 1 ? 'starter is' : 'starters are'} set for week ` +
        ...
        if (started.length) {
            ...
            for (const block of started)
                body.push(...previewBlock(block.m, block.variant, now), '');
        }
        if (upcoming.length) {
        ...
        if (rest > 0) {
            body.push(`${rest} further ${rest === 1 ? 'matchup rounds' : 'matchups round'} out the week ` +
                `${request.week} board.`);
        }
    }
    return {
...
const defaultComposer = (request) => {
    ...
    if (preview)
        return composePreview(request, heading, slate);
    ...
    if (!rows.length) {
        ...
        if (!board.length) {
            ...
            body.push(`Nothing in week ${request.week} turned a matchup. Every starter the math could place against ` +
