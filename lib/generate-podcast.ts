...
type Line = { host: 'MARK' | 'SULLY'; text: string };
...
export default async function handler(req: Request, res: Response) {
  ...
  if (!process.env.ELEVENLABS_API_KEY || !process.env.ELEVENLABS_MARK_VOICE_ID || !process.env.ELEVENLABS_SULLY_VOICE_ID) {
    return res.status(503).json({ error: 'Podcast voices are not configured yet' });
  }
  ...
  if (!/^\d{1,20}$/.test(leagueId) || !TOKEN_RE.test(token) ||
      ...
      !lines.every((line: Line) => line && (line.host === 'MARK' || line.host === 'SULLY') &&
  ...
  try {
    ...
    for (const line of lines as Line[]) {
      const voiceId = line.host === 'MARK' ? process.env.ELEVENLABS_MARK_VOICE_ID! : process.env.ELEVENLABS_SULLY_VOICE_ID!;
