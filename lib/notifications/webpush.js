...
/* A non-secret snapshot of the browser transport, the counterpart to
   apns.describe(). The private key is never returned, only whether it is
   present, so one diagnostic response can report both transports without
   either of them leaking a credential. */
function describe() {
  const c = vapidConfig();
  return {
    configured: isConfigured(),
    libraryLoaded: !!library(),
    publicKeyPresent: !!c.publicKey,
    privateKeyPresent: !!c.privateKey,
...
module.exports = { isConfigured, describe, publicKey, send, validSubscription };
