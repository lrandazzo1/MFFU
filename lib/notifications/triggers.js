/* ============================================================================
   ...
   Copy selection is seeded from the public NFL season + week + trigger, sliced
   into an index. No Math.random, no Date.now inside the copy path. The same
   public news edition always draws the same sentence, so the editorial payload
   is identical for every reader and never depends on a fantasy league id.
...
/* --------------------------------------------------------------------------
   ...
   payload carries only public-news routing metadata — season, week, trigger
   and generated-content slot. Device and league state select the recipient,
   but neither is embedded in or required to open the article.
-------------------------------------------------------------------------- */
