...
function buildNotification(triggerId, ctx) {
  ...
  return {
    ...
    data: {
      ...
      week: String(week),
      /* Every scheduled editorial alert opens the News Desk. This surface is
         public-feed backed and then adds league context only when the reader
         has a league loaded, so the route never depends on a league id. */
      url: '/?goto=news',
    },
  };
}
