...
/* --------------------------------------------------------------------------
   scoreboardUrl({ season, week, seasonType })

   The same scoreboard, asked for ONE specific week instead of whatever is
   current. `pull()` above wants today's week and takes no parameters; the
   article pipeline recaps a named week, including a backfill of a week that
   closed weeks ago, and has to ask for that week by number.

   seasontype 2 is the regular season, which is the only part of the calendar
   the article pipeline covers (its `week` is checked 1..18 everywhere else).
-------------------------------------------------------------------------- */
function scoreboardUrl(options) {
  const opts = options || {};
  const url = new URL(feedUrl());
  const season = Number(opts.season);
  const week = Number(opts.week);
  if (Number.isFinite(season) && season > 0) url.searchParams.set('dates', String(season));
  if (Number.isFinite(week) && week > 0) url.searchParams.set('week', String(week));
  url.searchParams.set('seasontype', String(Number(opts.seasonType) || 2));
  return url.toString();
}

/* --------------------------------------------------------------------------
   parseProTeamKickoffs(payload)

   Pure. Every NFL team that plays in this scoreboard document, mapped to the
   instant its game kicks off.

   ---- WHY THIS EXISTS ----

   `lib/article-math.ts` places a fantasy starter's points in time against the
   matchup: what the score was BEFORE his NFL game began. Without a kickoff for
   every starter it refuses to compute a margin and reports
   MISSING_KICKOFF_DATA rather than guessing, which is the right call — a
   wrong kickoff silently manufactures a deficit that never existed.

   The ESPN FANTASY league endpoint carries no kickoff times. It never has.
   So every starter came back with kickoff null, every matchup was unresolved,
   no outcome flag was ever assigned, and every published article read "No
   Swings To Report". This scoreboard is where those times actually live, and
   it is already fetched for the push dispatcher, on a host already on the
   allowlist.

   ---- THE KEYS ----

   Indexed by BOTH the numeric team id and the uppercase abbreviation, because
   the fantasy payload identifies a player's NFL team by `proTeamId` (numeric,
   matching ESPN's core team ids) while some shapes carry only an abbreviation.
   Both point at the same timestamp, so a lookup by either answers.

   A bye team is absent, not zero: it has no game this week, and a starter on
   bye scores nothing and cannot move a margin.
-------------------------------------------------------------------------- */
function parseProTeamKickoffs(payload) {
  const doc = (payload && typeof payload === 'object') ? payload : {};
  const events = Array.isArray(doc.events) ? doc.events : [];
  ...
  for (const event of events) {
    ...
    for (const competition of competitions.length ? competitions : [null]) {
      /* The competition's own date wins when present: a rescheduled game keeps
         the event's original date in some documents and corrects it here. */
      const stamp = (competition && competition.date) || event.date;
      if (!stamp) continue;
      const ts = new Date(stamp).getTime();
      if (!Number.isFinite(ts)) {
        console.warn('[FSNPush] unparseable kickoff stamp on scoreboard event ' +
          String(event.id) + ': ' + String(stamp));
        continue;
      }
...
/* --------------------------------------------------------------------------
   pullKickoffs({ season, week, fetchImpl, url })

   One request, one week, the kickoff index for it. Separate from `pull()`
   because that one is rate limited against a cached row shared by the whole
   install base, and this one is a per-run read of a specific historical week
   that the cache row cannot answer.

   Throws rather than returning an empty index: "the week has no games" and
   "the request failed" need different responses from the caller, and an empty
   index silently reproduces the exact MISSING_KICKOFF_DATA this function was
   written to remove.
-------------------------------------------------------------------------- */
async function pullKickoffs(options) {
  const opts = options || {};
  const doFetch = opts.fetchImpl || globalThis.fetch;
  if (typeof doFetch !== 'function') throw new Error('NO_FETCH_AVAILABLE');

  const url = opts.url || scoreboardUrl(opts);
  ...
  if (!response || !response.ok) throw new Error('KICKOFF_FEED_HTTP_' + String(response && response.status));
  ...
  try {
    ...
    throw new Error('KICKOFF_FEED_BODY_NOT_JSON');
  }

  const index = parseProTeamKickoffs(payload);
  if (!Object.keys(index).length) throw new Error('KICKOFF_FEED_NO_GAMES');
  return index;
}
