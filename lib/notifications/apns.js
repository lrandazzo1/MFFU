/* ============================================================================
   ...
   anything the app or this file does at runtime, and the two must agree or
   every push is rejected with BadDeviceToken:

     aps-environment = production   App Store, TestFlight, and any
                                    ad-hoc/enterprise export  -> api.push.apple.com
     aps-environment = development  a build run from Xcode onto an attached
                                    device                    -> api.sandbox.push.apple.com

   TestFlight is a PRODUCTION signature. `ios/App.entitlements` pins
   `aps-environment` to `production` and `scripts/verify-ios-release.py`
   asserts it on the exported archive, so the default `APNS_ENV=production`
   here is the setting that matches a TestFlight install. Setting APNS_ENV to
   'sandbox' while testing through TestFlight is the classic silent failure:
   Apple accepts the connection, then rejects each token as BadDeviceToken.
   describe() reports both halves so the pairing can be read off one response.
============================================================================ */
...
/* --------------------------------------------------------------------------
   ...
   A non-secret snapshot of how this deployment is pointed at Apple, for the
   dispatcher's diagnostics. It exists because the two most common reasons a
   correctly-built app receives nothing are both invisible from a send's
   response alone:

     * APNS_ENV disagrees with the aps-environment the binary was signed with
       (a TestFlight build is signed `production`), which Apple answers with a
       flat BadDeviceToken per device and no hint as to why;
     * APNS_KEY_P8 arrived mangled — pasted without its PEM header, or with
       the newlines eaten — which throws inside crypto.sign at the first send
       rather than at configuration time.

   The private key itself is NEVER returned, only whether it parses and signs.
   keyId and teamId ARE returned in full: both travel in the clear inside every
   provider JWT header Apple receives, neither authenticates anything without
   the key, and being able to compare them against the AuthKey filename is the
   point of the check. The route that exposes this is CRON_SECRET-gated.
-------------------------------------------------------------------------- */
function describe() {
  const c = apnsConfig();
  const env = c.host === SANDBOX_HOST ? 'sandbox' : 'production';

  let keyFormat = 'missing';
  if (c.key) {
    keyFormat = /-----BEGIN [A-Z ]*PRIVATE KEY-----/.test(c.key) ? 'pem' : 'unrecognised';
  }

  /* Sign one fixed string. A key that cannot sign here cannot sign a provider
     token either, and finding that out now beats finding it out as an opaque
     throw in the middle of a fan-out. */
  let keyUsable = false;
  let keyError = null;
  if (c.key) {
    try {
      crypto.sign('sha256', Buffer.from('fsn-apns-key-probe'), { key: c.key, dsaEncoding: 'ieee-p1363' });
      keyUsable = true;
    } catch (err) {
      keyError = String((err && err.message) || err).slice(0, 200);
      console.error('[FSNPush] APNS_KEY_P8 is present but will not sign. Every push ' +
        'from this deployment would fail. Check that the full PEM block was pasted, ' +
        'header and footer included.', err);
    }
  }
  ...
  return {
    ...
    env,
    host: c.host,
    /* What goes out as the apns-topic header. It must equal the app's bundle
       id exactly or Apple answers DeviceTokenNotForTopic. */
    topic: c.bundleId,
    keyId: c.keyId || null,
    ...
    keyUsable,
    keyError,
    /* Stated rather than implied, because the pairing is the single most
       common misconfiguration in this stack. */
    matches: env === 'production'
      ? 'TestFlight / App Store builds (aps-environment=production)'
      : 'Xcode development builds only (aps-environment=development) — a TestFlight token will be rejected as BadDeviceToken',
  };
}
...
module.exports = { isConfigured, describe, openSession, closeSession, send, apnsConfig };
